...
void Folder::addFileToArray(const Folder& nf) throw(const char*)
{
	...
	for (int i = 0; i < this->fSize; i++) {
		temp[i] = new Folder(*this->f[i], this->f[i]->path);
	}
	delete[] this->f;
	for (int i = 0; i < this->fSize; i++) {
		this->f[i] = new Folder(*temp[i],temp[i]->path);
		delete temp[i];
	}
	delete[] temp;
	this->fSize++;

	this->f = new Folder*[this->fSize];
	this->f[this->fSize - 1] = new Folder(nf,this->FileName);
...
void Folder::dir() const
{
	...
	for (int i = 0; i < this->dfSize; i++) {
		cout << this->getTime() << "  " << "<DIR>" << "  " << fixed << setprecision(2) << (float)this->df[i]->getSize() / 1000 << "KB" << "  " << this->df[i]->getFileName();
	}
